...
import { loadManifest } from "./src/assets/media-downloader.js";
import { buildMergedSrt } from "./src/assets/srt-merge.js";
...
async function main() {
  ...
  // Article media manifest (from a previous full pipeline run) → resolve
  // $media.N placeholders + auto-illustrate plain body scenes.
  const manifest = (await loadManifest(outputDir)).filter((e) => existsSync(join(outputDir, e.relPath)));
  const map = new Map(manifest.map((e) => [e.placeholder, e]));
  for (const scene of script.scenes) {
    const td = scene.templateData as Record<string, unknown>;
    for (const key of ["bgSrc", "videoSrc"]) {
      const v = td[key];
  ...
  let pi = 0;
  for (const scene of script.scenes) {
    const t = scene.templateData.template;
    if (scene.type !== "body") continue;
    if (t !== "stat-hero" && t !== "feature-list" && t !== "callout") continue;
    if ((scene.templateData as { bgSrc?: string }).bgSrc) continue;
    if (pi >= pool.length) break;
    sceneBg[scene.id] = pool[pi].relPath;
    console.log(`  auto-illustrate ${scene.id} ← ${pool[pi].relPath}`);
    pi += 1;
  }

  // Merged timeline-accurate subtitles
  const mergedSrt = await buildMergedSrt(
    sceneAudio.map((a) => ({ id: a.id, durationSec: a.durationSec })),
    join(outputDir, "voice"),
    SCENE_GAP_SEC,
  );
  if (mergedSrt) {
